...
void Class_PID::Init(float __K_P, float __K_I, float __K_D, float __I_Out_Max, float __Out_Max)
{
    ...
    /*
     * 每次重新初始化 PID 都清空内部历史状态。
     * 这样切换测试轮子或者重新调参后不会继承旧的积分和误差。
     */
    Integral_Error = 0.0f;
    Error = 0.0f;
...
void Class_PID::Adjust_TIM_PeriodElapsedCallback()
{
    ...
    {
        float Speed_Ratio;
        if (Variable_Speed_I_A == 0 && Variable_Speed_I_B == 0)
        {
            Speed_Ratio = 1;
        }
        else
        {
            ...
            else if (Variable_Speed_I_B <= Abs_Error && Abs_Error < Variable_Speed_I_A + Variable_Speed_I_B)
            {
