...
//PID定时器周期, s
/*
 * 电机速度环 / 角度环每 20ms 执行一次，
 * 因此 PID 积分时间步长同步为 0.02s。
 */
const float PID_CALCULATE_PERIOD = 0.02f;
...
class Class_PID
{
    public:
        ...
        void Init(float __K_P, float __K_I, float __K_D, float __I_Out_Max, float __Out_Max);//Out_Max 输出的PWM最大值（即ARR）
